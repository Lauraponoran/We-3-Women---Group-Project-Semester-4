...
def clean_topic_name(name: str) -> str:
    """Converts '12_women_safety_protest' to 'Women, Safety, Protest'."""
    if not isinstance(name, str) or name == "-1": return "Outliers"
    parts = name.split('_')
    # Skip the leading ID number
    start_idx = 1 if parts[0].replace('-', '').isdigit() else 0
    words = parts[start_idx:start_idx+3] 
    return ", ".join(w.capitalize() for w in words)

def is_feminist(name: str) -> bool:
    """Flags a topic as feminist based on keywords."""
...
def chart_topics_by_event(df: pd.DataFrame, out_dir: str, dpi: int, top_n: int) -> None:
    ...
    for ax, event in zip(axes, events):
        ...
        # Rescue logic: Top N + all feminist topics present in this event
    ...
    fig.suptitle("What Filled the News Cycle?\nTop Topics Per Protest Event", fontsize=16, fontweight="bold", y=1.02)
    plt.tight_layout()
...
def chart_sentiment_by_topic(df: pd.DataFrame, out_dir: str, dpi: int, top_n: int) -> None:
    ...
    agg = agg[agg["friendly_name"] != "Outliers"]
    
    # Rescue logic: Top N by volume + all feminist topics
    ...
    ax.set_title("Which Topics Get Framed Most Negatively?\nMean Sentiment by Topic (Sized by Volume)", fontweight="bold", fontsize=14)
...
def chart_topic_pie(df: pd.DataFrame, out_dir: str, dpi: int, top_n: int) -> None:
    counts = df[df["friendly_name"] != "Outliers"]["friendly_name"].value_counts()
    
    top = counts.head(top_n)
    rest = counts.iloc[top_n:]
    
    # Rescue feminist topics from "Other"
    ...
    for lbl in slices.index:
        if is_feminist(lbl):
            ...
            explode.append(0.1) # Pop feminist topics out
    ...
    # Make feminist labels bold
    for i, lbl in enumerate(slices.index):
        if is_feminist(lbl):
            texts[i].set_fontweight('bold')
            texts[i].set_color(FEMINIST_OUTLINE)

    ax.set_title("Overall News Coverage by Topic", fontsize=16, fontweight="bold")
...
def main():
    ...
    # Map Friendly Names using topic_info.csv
    ...
    print(f"Success! All 4 figures saved to {args.out_dir}")
