...
# ══════════════════════════════════════════════════════════════════════════════
# ❹  CHART 3 — PROTEST VS CONTROL SENTIMENT
# ══════════════════════════════════════════════════════════════════════════════

def _chart_protest_only(merged: pd.DataFrame, out_dir: str, dpi: int) -> None:
    """Fallback: simple horizontal bar of protest-week sentiment per publisher."""
    fig, ax = plt.subplots(figsize=(10, max(5, len(merged) * 0.55)))
    _base_style(fig, ax)

    colors = [RASPBERRY if v < 0 else OLIVE for v in merged["sentiment_score"]]
    bars = ax.barh(merged["publisher"], merged["sentiment_score"],
                   color=colors, edgecolor="white", linewidth=0.5, height=0.65)

    for bar, val in zip(bars, merged["sentiment_score"]):
    ...
    ax.axvline(0, color=DARK_PLUM, linewidth=1.2)
    ax.set_xlabel("Mean VADER Sentiment Score (protest weeks)", fontsize=11)
    ax.set_title("Sentiment During Protest Weeks by Publisher\n"
                 "(no matched control weeks found in data)",
                 fontsize=14, fontweight="bold", pad=14)

    plt.tight_layout()
    path = os.path.join(out_dir, "protest_vs_control_sentiment.png")
    fig.savefig(path, dpi=dpi, bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.close(fig)
...
def chart_protest_vs_control(df: pd.DataFrame, out_dir: str, dpi: int) -> None:
    # Diagnostic — print what values is_control actually contains
    print(f"    is_control unique values: {df['is_control'].unique()}")
    print(f"    is_control dtype: {df['is_control'].dtype}")
    print(f"    protest rows : {(~df['is_control']).sum()}")
    print(f"    control rows : {df['is_control'].sum()}")

    # Coerce to bool robustly (handles True/False, 1/0, 'True'/'False')
    def to_bool(val):
    ...
    df = df.copy()
    df["is_control"] = df["is_control"].apply(to_bool)

    agg = (df.groupby(["publisher", "is_control"])["sentiment_score"]
             .mean().reset_index())

    protest = agg[~agg["is_control"]].rename(columns={"sentiment_score": "protest"})
    control = agg[ agg["is_control"]].rename(columns={"sentiment_score": "control"})
    merged  = protest.merge(control[["publisher", "control"]], on="publisher", how="inner")

    # Fallback: if no publisher appears in both groups, show protest-only bars
    if merged.empty:
        print("    ⚠️  No publishers found in BOTH protest and control weeks.")
        print("       Falling back to protest-week sentiment only (no comparison).")
        merged = protest.rename(columns={"protest": "sentiment_score"})
        merged = merged.sort_values("sentiment_score")
        _chart_protest_only(merged, out_dir, dpi)
        return

    merged["diff"] = merged["protest"] - merged["control"]
    merged = merged.sort_values("diff")

    fig, ax = plt.subplots(figsize=(11, max(5, len(merged) * 0.65)))
    _base_style(fig, ax)

    y = np.arange(len(merged))
    ax.scatter(merged["control"], y, color=BLUSH,   s=120, zorder=3,
               label="Control week", edgecolors=RASPBERRY, linewidths=1.2)
    ax.scatter(merged["protest"], y, color=DEEP_PURPLE, s=120, zorder=3,
               label="Protest week", edgecolors=DARK_PLUM, linewidths=1.2)

    for i, row in enumerate(merged.itertuples()):
        color = RASPBERRY if row.protest < row.control else OLIVE
        ax.plot([row.control, row.protest], [i, i],
                color=color, linewidth=2, zorder=2, alpha=0.8)
        ax.text(max(row.protest, row.control) + 0.01, i,
                f"  Δ{row.diff:+.2f}", va="center", fontsize=8.5,
                color=color, fontweight="bold")

    ax.set_yticks(y)
    ax.set_yticklabels(merged["publisher"], fontsize=11)
    ax.axvline(0, color=DARK_PLUM, linewidth=1, linestyle="--", alpha=0.5)
    ax.set_xlabel("Mean VADER Sentiment Score", fontsize=11)
    ax.set_title("Does Protest Coverage Get More Negative?\n"
                 "Sentiment During Protest Weeks vs Matched Control Weeks",
                 fontsize=14, fontweight="bold", pad=14)
    ax.legend(facecolor=LAVENDER_CREAM, edgecolor=BLUSH,
              labelcolor=DARK_PLUM, fontsize=10)
...
def main() -> None:
    ...
    print("[3/3] Protest vs control sentiment ...")
    chart_protest_vs_control(sent_df, args.out_dir, args.dpi)
