...
DEFAULT_TOPICS    = os.path.join("analysis_output", "topic_info.csv")
...
def _coerce_topic_labels(df: pd.DataFrame) -> pd.DataFrame:
    """
    Ensure topic_label is a plain Python string column.

    Pandas Categorical dtypes cause silent failures in .isin() comparisons
    when the test values are not already in the category set.  Converting to
    str fixes the feminist-topic rescue logic in all four chart functions.
    """
...
def main() -> None:
    ...
    # ── FIX: coerce topic_label to plain str so .isin() works reliably ────────
    # Categorical dtypes cause silent failures when the tested values (e.g.
    # "ich_ruth_frauen_familia") are not already in the category index.
    topic_df = _coerce_topic_labels(topic_df)

    # Print a quick diagnostic so you can confirm the feminist topics are present
    fem_found = topic_df["topic_label"].isin(FEMINIST_TOPICS).sum()
    print(f"  Feminist-topic rows found: {fem_found:,}  "
          f"(labels: {FEMINIST_TOPICS})")
    if fem_found == 0:
        print("  ⚠️  WARNING: no feminist-topic rows found — check FEMINIST_TOPICS "
              "strings match your data exactly.")
        print("  Unique topic_label sample:")
        print(" ", list(topic_df["topic_label"].unique()[:15]))
