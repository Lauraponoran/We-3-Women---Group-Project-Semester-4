...
def chart_topic_pie(df: pd.DataFrame, out_dir: str, dpi: int, top_n: int) -> None:
    ...
    Top N topics by article count are shown individually; the rest are
    grouped into an 'Other topics' slice to keep the chart readable.
    """
    ...
    # Top N slices + group the rest
    top    = counts.head(top_n)
    other_n = counts.iloc[top_n:]["n"].sum()

    if other_n > 0:
        other_row = pd.DataFrame([{"topic_label": "Other topics", "n": other_n}])
        slices = pd.concat([top, other_row], ignore_index=True)
    else:
        slices = top

    # Palette: cycle through purple shades, grey for "Other"
    pie_colors = [PALETTE[i % len(PALETTE)] for i in range(len(slices))]
    if other_n > 0:
        pie_colors[-1] = "#C9B8D4"   # muted lavender for the catch-all slice

    # Slightly explode the largest slice for emphasis
    explode = [0.04 if i == 0 else 0 for i in range(len(slices))]
    ...
    wedges, texts, autotexts = ax.pie(
        ...
        pctdistance=0.78,
        startangle=140,
        wedgeprops={"edgecolor": LAVENDER_CREAM, "linewidth": 1.5},
    )
