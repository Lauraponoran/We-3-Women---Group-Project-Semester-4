...
PARALLELISM NOTES
-----------------
Publishers are crawled concurrently using a ThreadPoolExecutor (I/O-bound).
--workers  controls how many publishers run in parallel (default 8).
--processes controls CCNewsCrawler internal processes per publisher (default 1).
  Keep processes=1 when workers>1 to avoid spawning too many subprocesses.
A per-publisher timeout (--pub-timeout, default 120s) prevents a single
slow/empty publisher from blocking a worker slot indefinitely.
Progress saving is protected by a threading.Lock so concurrent completions
...
Usage:
    ...
    python collect_articles.py --workers 8                       # parallel publishers
    python collect_articles.py --processes 1                     # CCNews internal procs
    python collect_articles.py --pub-timeout 120                 # per-publisher timeout (s)
"""
...
DEFAULT_WORKERS         = 8    # parallel publishers per event
DEFAULT_PROCESSES       = 1    # CCNewsCrawler internal processes (keep low when workers>1)
DEFAULT_PUB_TIMEOUT     = 120  # seconds before a publisher crawl is abandoned
...
# Global lock for thread-safe progress file writes
...
# ══════════════════════════════════════════════════════════════════════════════
# ❻  PROGRESS  (thread-safe reads/writes)
# ══════════════════════════════════════════════════════════════════════════════
...
def save_progress(prog: dict) -> None:
    """Write progress JSON; must be called while holding _progress_lock."""
...
# ══════════════════════════════════════════════════════════════════════════════
# ❽  PROCESS ONE EVENT  (publishers crawled in parallel)
# ══════════════════════════════════════════════════════════════════════════════

def process_event(event: dict, progress: dict,
                  per_publisher: int, control_per_publisher: int,
                  processes: int, workers: int, pub_timeout: int) -> bool:
    label      = event["label"]
    ...
    print(f"Cap     : {cap} articles/publisher  |  workers={workers}  timeout={pub_timeout}s")
    if event.get("note"):
    ...
    # ── Parallel crawl ────────────────────────────────────────────────────────
    pbar = tqdm(total=len(todos), desc="  Publishers", unit="pub", leave=False)

    def _save_publisher_result(publisher, records: list[dict]) -> None:
        """Persist one publisher's results and update shared progress (thread-safe)."""
        pub_name = str(publisher).split(".")[-1]
        n = len(records)
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(future_to_pub):
            ...
            pub_name = str(pub).split(".")[-1]
            try:
                records = future.result(timeout=pub_timeout)
            except TimeoutError:
                pbar.write(f"    ⏱  {pub_name:<28s}  timed out after {pub_timeout}s")
                records = []
            except Exception as exc:
                pbar.write(f"    ❌  {pub_name:<28s}  error: {exc}")
                records = []

            _save_publisher_result(pub, records)
...
def main() -> None:
    ...
    parser.add_argument("--workers",                 type=int, default=DEFAULT_WORKERS,
                        help=f"Parallel publishers per event (default {DEFAULT_WORKERS})")
    parser.add_argument("--processes",               type=int, default=DEFAULT_PROCESSES,
                        help=f"CCNewsCrawler internal processes per publisher (default {DEFAULT_PROCESSES})")
    parser.add_argument("--pub-timeout",             type=int, default=DEFAULT_PUB_TIMEOUT,
                        help=f"Per-publisher timeout in seconds (default {DEFAULT_PUB_TIMEOUT})")
    ...
    print(f"  CCNews processes/pub    : {args.processes}")
    print(f"  Per-publisher timeout   : {args.pub_timeout}s")
